...
must_haves:
  truths:
    ...
    - "While Rehearsals data loads, the page shows shimmer skeleton rows (no spinner visible)"
    - "When Rehearsals list is empty, the page shows a structured empty state with icon and CTA button"
  artifacts:
...
4. Replace the inline empty state (~lines 700-718) with `EmptyState`.
   ...
   The EmptyState component interface (from `src/components/feedback/EmptyState.tsx`) is:
   - `icon?: React.ReactNode` — expects a rendered JSX element, NOT a component reference
   - `title: string`
   - `description?: string`
   - `action?: { label: string; onClick: () => void }` — single object, NOT separate props
