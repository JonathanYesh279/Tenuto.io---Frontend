          ...
          <AnimatePresence initial={false} mode="wait">
            {boundsState && (
              <motion.div
                ...
                animate={{ top: boundsState.top, left: boundsState.left, width: boundsState.width, height: boundsState.height, opacity: 1 }}
                initial={{ top: boundsState.top, left: boundsState.left, width: boundsState.width, height: boundsState.height, opacity: 0 }}
                exit={{ opacity: 0, transition: { ...transition, delay: ((transition as any)?.delay ?? 0) + (exitDelay ?? 0) / 1000 } }}
                transition={transition}
...
function HighlightItem<T extends React.ElementType>({
  ...
  const {
    activeValue, setActiveValue, mode, setBounds, clearBounds,
    hover, click, enabled, className: contextClassName, style: contextStyle,
    transition: contextTransition, id: contextId, disabled: contextDisabled,
    exitDelay: contextExitDelay, setActiveClassName,
  ...
  const commonHandlers = hover
    ? {
        onMouseEnter: (e: React.MouseEvent<HTMLDivElement>) => { setActiveValue(childValue); element.props.onMouseEnter?.(e); },
        onMouseLeave: (e: React.MouseEvent<HTMLDivElement>) => { setActiveValue(null); element.props.onMouseLeave?.(e); },
      }
    : click
      ? { onClick: (e: React.MouseEvent<HTMLDivElement>) => { setActiveValue(childValue); element.props.onClick?.(e); } }
      : {};
  ...
  if (asChild) {
    if (mode === 'children') {
      return React.cloneElement(element, {
        ...
        ...getNonOverridingDataAttributes(element, { ...dataAttributes, 'data-slot': 'motion-highlight-item-container' }),
        ...commonHandlers,
        ...props,
      } as any, <>
        <AnimatePresence initial={false} mode="wait">
          {isActive && !isDisabled && (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0, transition: { ...itemTransition, delay: ((itemTransition as any)?.delay ?? 0) + (exitDelay ?? contextExitDelay ?? 0) / 1000 } }}
              {...dataAttributes}
            />
          )}
        </AnimatePresence>
