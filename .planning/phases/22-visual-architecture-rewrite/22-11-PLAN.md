...
In TeacherTabNavigation.tsx, StudentTabNavigation.tsx:
...
**OrchestraTabNavigation.tsx (onClick-based tab pattern — NOT NavLink):**
Orchestra tabs use onClick-based switching (not React Router NavLink). Apply the SAME visual styling:
- Replace `border-primary-500 text-primary-600` (active) → `border-foreground text-foreground font-semibold border-b-2`
- Replace `border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300` (inactive) → `border-transparent text-muted-foreground hover:text-foreground`
- Keep the onClick handler and state-based tab switching logic unchanged — only restyle the tab buttons
- Tab container: `flex gap-6` — same as teacher/student tabs

**OrchestraTabContent.tsx:**
- Replace `text-gray-500` → `text-muted-foreground`
- No Card wrappers in tab content areas
...
4. Grep for `lucide-react` in TeacherTabNavigation.tsx, StudentTabNavigation.tsx, OrchestraTabNavigation.tsx — should be ZERO
5. Grep for `border-primary-500` and `text-gray-500` in OrchestraTabNavigation.tsx — should be ZERO
6. Grep for `text-gray-500` in OrchestraTabContent.tsx — should be ZERO
  </verify>
  <done>All three entity detail pages have identity block with attached tab bar. Tab content is a continuous document feel. No pastel card headers. No floating tab bars. Orchestra onClick-based tabs restyled with same semantic tokens. Dossier archetype achieved.</done>
</task>
