...
Color rationale:
- `--primary: 15 85% 55%` → deep coral/salmon (hue 15, highly saturated, medium lightness) — WCAG AA contrast 4.5:1+ on white
...
2. `grep '15 85% 55%' src/index.css` confirms coral primary value (not blue 245 58% 51%)
...
9. Verify no `@import.*Inter` remains in src/index.css
  </verify>
  <done>
CSS custom properties defined in :root via @layer base in index.css with WARM CORAL palette values (--primary: 15 85% 55%, --background: warm off-white, --sidebar: dark charcoal/navy, --accent: warm amber). Tailwind config merges semantic tokens (DEFAULT, foreground) into existing primary/secondary scales without breaking bg-primary-500 class resolution. New semantic keys (background, foreground, border, etc.) added. Heebo is first in font stack (both Tailwind config and index.css html element). borderRadius uses CSS variable. Build passes.
  </done>
</task>
...
<success_criteria>
- CSS custom properties exist in :root with warm coral/amber HSL channel values for all shadcn semantic tokens (FNDTN-06 satisfied)
